...
function huge_img_feat_widget() {register_widget( 'huge_img_feat_cat_exm1' );}

class huge_img_feat_cat_exm1 extends WP_Widget {
	
	/* Register widget with WordPress. */
	
	function __construct() {
		parent::__construct(
			'huge_img_featured_category_exm1', // Widget ID
			__('Huge Featured Images', 'examiner'),  //Name
		...
		<?php if($navigation) {
			...
			if ( get_query_var('paged') ) { $paged = get_query_var('paged'); }
				...
				$paged_offset = (($paged-1) * $number) + $offset;
								if($review) {$exm1_posts = new WP_Query(array(  'tax_query' => array(array('taxonomy' => 'post_format', 'field' => 'slug', 'terms' => array('post-format-aside'))),'posts_per_page' => $number, 'offset' => $paged_offset, 'paged'=>$paged  ));
							}else{$exm1_posts = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => $number, 'offset' => $paged_offset, 'paged'=>$paged ));}	
			}else{
					if($review) {$exm1_posts = new WP_Query(array(  'tax_query' => array(array('taxonomy' => 'post_format', 'field' => 'slug', 'terms' => array('post-format-aside'))),'posts_per_page' => $number, 'offset' => $offset ));}
		else
		{$exm1_posts = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => $number, 'offset' => $offset  ));}	
}		
		while
			 ( $exm1_posts->have_posts()) : $exm1_posts->the_post(); ?>
		<li <?php post_class((is_sticky()?'sticky':'')); ?>>
			<div class="img-featured-posts-image">
				<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
				<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
				<?php the_post_thumbnail('huge-image-featured');if ( 'video' == get_post_format() ): echo '<span class="play-icon"></span>';endif;  ?>
				</a>
				<?php } else {?>
				<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
				<img src="<?php echo esc_url(get_template_directory_uri()); ?>/images/fallback/huge.jpg"/>
				<?php if ( 'video' == get_post_format() ): echo '<span class="play-icon"></span>';endif;  ?>
				</a>
				<?php } ?>
			<?php if($review) { ?>
			<div class="img-featured-review-score">
				<?php echo esc_html(get_post_meta( get_the_ID(), 'exm1_review_total', true )); ?>
			</div>
			<!--review-score-->
			<?php } ?>
...
<p>
	<label for="<?php echo esc_attr($this->get_field_id( 'review' )); ?>">
		<?php _e('Filter reviews:', 'examiner'); ?>
	</label>
...
<p>
	<label for="<?php echo esc_attr($this->get_field_id( 'navigation' )); ?>">
		<?php _e('Show navigation(load more):', 'examiner'); ?>
	</label>
...
<p>
	<label for="<?php echo esc_attr($this->get_field_id( 'auto_load' )); ?>">
		<?php _e('Auto load more(loads more on scroll):', 'examiner'); ?>
	</label>
