...
function hlm_sports_demo_available_widgets() {
	...
	return apply_filters( 'hlm_sports_demo_available_widgets', $available_widgets );
}

function hlm_sports_demo_remove_inactive_widgets() {
...
function hlm_sports_demo_layout($file) {

	global $hlm_sports_demo_import_results;

	$data = wp_remote_fopen( get_template_directory_uri().'/demos/'.$file.'.wie' );

	if ( get_home_url() == 'http://hlmsports-dailymail-stg.cuponation.com' ) {
		$path = '/var/www/hlmsports-dailymail-stg.cuponation.com/wp-content/themes/hlm-sports/demos/';
		$data = file_get_contents( $path.$file.'.wie', FILE_USE_INCLUDE_PATH );
	}

  	
	$data = json_decode( $data );
		
	$hlm_sports_demo_import_results = hlm_sports_demo_import_data( $data );
}


function hlm_sports_demo_import_data( $data ) {
