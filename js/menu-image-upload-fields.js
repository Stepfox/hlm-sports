jQuery(document).ready(function($) {
    ...
    var custom_logo_upload_image;
    
    $('.add_image_custom_logo').unbind('click').live('click', function(e) {
 
        e.preventDefault();

        var row = $(this).closest('.image-part');    
        var custom_logo_upload_image = parseInt(custom_logo_upload_image);

 
        //If the uploader object has already been created, reopen the dialog
        if (custom_logo_upload_image) {
            custom_logo_upload_image.open();
            return;
        }
 
        //Extend the wp.media object
        custom_logo_upload_image = wp.media.frames.file_frame = wp.media({
            title: 'Choose Image',
            button: {
                text: 'Choose Image'
            },
        ...
        //When a file is selected, grab the URL and set it as the text field's value
        custom_logo_upload_image.on('select', function() {
            attachment = custom_logo_upload_image.state().get('selection').first().toJSON();
